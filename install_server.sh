...
function create_persisted_state_dir() {
  ...
  readonly WIN_STATE_DIR="${SHADOWBOX_WIN_DIR}/persisted-state"
  mkdir -p "${STATE_DIR}"
...
function start_shadowbox() {
  ...
  local -ar docker_shadowbox_flags=(
    --name "${CONTAINER_NAME}" --restart always -p 0.0.0.0:${API_PORT}:${API_PORT}/tcp -p 0.0.0.0:${FLAGS_KEYS_PORT}:${FLAGS_KEYS_PORT}/tcp -p 0.0.0.0:${FLAGS_KEYS_PORT}:${FLAGS_KEYS_PORT}/udp
    --label 'com.centurylinklabs.watchtower.enable=true'
    -v "${WIN_STATE_DIR}:${STATE_DIR}"
